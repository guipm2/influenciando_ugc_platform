  ...
  const fetchDashboardStats = useCallback(async () => {
    ...
    try {
      ...
      const { data: conversationsData } = await supabase
        .from('conversations')
        .select('opportunity_id, creator_id, last_message_at')
      ...
      const projectsWithDeliverables = (projectsData || []).map(project => {
        ...
        // Add deliverable updates
        if (projectDeliverables.length > 0) {
          projectDeliverables.forEach(d => {
            timestamps.push(new Date(d.updated_at || d.created_at).getTime());
          });
        }

        // Add conversation activity
        // Note: conversations are identified by opportunity_id + creator_id
        const conversation = (conversationsData || []).find(c =>
          c.opportunity_id === opportunity.id &&
          c.creator_id === project.creator_id
        );

        if (conversation?.last_message_at) {
          timestamps.push(new Date(conversation.last_message_at).getTime());
        }
        ...
        if (status === 'completed' && projectDeliverables.length > 0) {
                      ...
                      <div className="p-4 bg-gray-50 rounded-lg">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="font-medium">{stat.avg_duration} dias</span>
                          </div>
                        </div>
                      </div>
                    </div>
